function [modelPara, compPara, pcptPara] = UpdateBetaTheta(dimPara, hyperPara, optPara, modelPara, compPara, pcptPara, X1S, X2, Q)
...
%             etaProp(t, i) = qNorm(t, :, i) * etaTrue(:, t, i); %weighted sum
%             omega(1, t) = PolyaGamRnd(qSum(t, i), etaProp(t, i), trunc);
% %             no need to draw from PG dist again?
%             invV_theta  = invV_theta + omega(1, t) * X2TimesX2(:, :, t, i);
%         end
%         m_theta(i, :) = (invV_theta \ ...
%             (X2AvgQNorm(:, :, i)' * (qNJ(:, i) - qSum(:, i) / 2) - ...
%             X2AvgQNorm(:, :, i)' * diag(omega) * X1AvgQNorm(:, :, i) * beta(i, :)' + ...
%             invSB_theta))';       %Changed from betaNew to beta
