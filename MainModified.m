...
% running one instead of 4 experiments 
flagMH = repmat(0, 1, numData); %1 = Train, 0 = Polya-Gamma
flagSP = repmat(-1, 1, numData); % -1 = normal prior on theta, 0 = sparse prior on theta, 1 = sparse prior on b_theta
flagCS = repmat(0, 1, numData); % 1 = common sparse prior, 0 = customer-specific sparse prior. this switch is ignored if flagSP = -1.
...
%     n_collect = 2e3 * ones(1, numRun); % samples to be saved%$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
    n_collect = 1 * ones(1, numRun); % samples to be saved%$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
    iterDisplay = 100;
end
...
%         fullOutPath = fullfile(outPath{iRun}, DPM_MNL(optPara));
        [a,b]= DPM_MNL_runtime(optPara);
%         fullfile(outPath{iRun}, DPM_MNLMod(optPara));
        fid = fopen('log.txt', 'a');
